...
def normalize_angle_signed(angle):
    ...
    # Ensure π is included instead of -π
    return normalized if normalized != -math.pi else math.pi
...
def linearGradient(colors, normalizedZero2One):
    normalizedZero2One = clamp(0, 1 - 1e-3, normalizedZero2One)
...
def normalize(value, minValue, maxValue, doesCap=True):
